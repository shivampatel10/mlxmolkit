...
_MSL_SOURCE_TG = """
    ...
    // Copy initial positions (parallel across threads)
    ...
    // Shared BFGS state — thread 0 writes, all threads read after barrier
    threadgroup float tg_energy_shared;
    threadgroup float tg_lam;
    threadgroup float tg_prev_lam;
    threadgroup float tg_prev_e;
    threadgroup int tg_ls_done;
    threadgroup int tg_status_shared;
    threadgroup float tg_slope;
    threadgroup float tg_lambda_min;
    threadgroup float tg_old_energy;
    ...
    // Energy+gradient macros — thread 0 only (gradient funcs use non-atomic +=)
    ...
    // Energy-only (line search) — thread 0 only
    #define SEQ_COMPUTE_E(OUT_E) \\
        if (tid == 0) { \\
            OUT_E = 0.0f; \\
            for (int t = b_s; t < b_e; t++) OUT_E += bond_stretch_e(out_pos, bond_pairs[t*2], bond_pairs[t*2+1], bond_params[t*2], bond_params[t*2+1]); \\
            for (int t = a_s; t < a_e; t++) OUT_E += angle_bend_e(out_pos, angle_trips[t*3], angle_trips[t*3+1], angle_trips[t*3+2], angle_params[t*3], angle_params[t*3+1], angle_params[t*3+2]>0.5f); \\
            for (int t = sb_s; t < sb_e; t++) OUT_E += stretch_bend_e(out_pos, sb_trips[t*3], sb_trips[t*3+1], sb_trips[t*3+2], sb_params[t*5], sb_params[t*5+1], sb_params[t*5+2], sb_params[t*5+3], sb_params[t*5+4]); \\
            for (int t = o_s; t < o_e; t++) OUT_E += oop_bend_e(out_pos, oop_quads[t*4], oop_quads[t*4+1], oop_quads[t*4+2], oop_quads[t*4+3], oop_params[t]); \\
            for (int t = t_s; t < t_e; t++) OUT_E += torsion_e(out_pos, tor_quads[t*4], tor_quads[t*4+1], tor_quads[t*4+2], tor_quads[t*4+3], tor_params[t*3], tor_params[t*3+1], tor_params[t*3+2]); \\
            for (int t = v_s; t < v_e; t++) OUT_E += vdw_e(out_pos, vdw_pairs[t*2], vdw_pairs[t*2+1], vdw_params[t*2], vdw_params[t*2+1]); \\
            for (int t = e_s; t < e_e; t++) OUT_E += ele_e(out_pos, ele_pairs[t*2], ele_pairs[t*2+1], ele_params[t*3], (int)ele_params[t*3+1], ele_params[t*3+2]>0.5f); \\
        } \\
        threadgroup_barrier(mem_flags::mem_device);

    // ---- Initial energy + gradient (thread 0) ----
    if (tid == 0) tg_energy_shared = 0.0f;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    float energy_local = 0.0f;
    SEQ_COMPUTE_EG(energy_local);
    if (tid == 0) tg_energy_shared = energy_local;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    // Max step (parallel reduction)
    tg_reduce[tid] = 0.0f;
    for (int i = (int)tid; i < n_terms; i += (int)tg_size) tg_reduce[tid] += my_pos[i]*my_pos[i];
    threadgroup_barrier(mem_flags::mem_threadgroup);
    for (uint s = tg_size/2; s > 0; s >>= 1) { if (tid < s) tg_reduce[tid] += tg_reduce[tid+s]; threadgroup_barrier(mem_flags::mem_threadgroup); }
    float max_step = MAX_STEP_FACTOR * max(sqrt(tg_reduce[0]), (float)n_terms);

    if (tid == 0) tg_status_shared = 1;
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (int iter = 0; iter < max_iters && tg_status_shared == 1; iter++) {
        // Save old pos (ALL threads)
        for (int i = (int)tid; i < n_terms; i += (int)tg_size) my_old_pos[i] = my_pos[i];
        threadgroup_barrier(mem_flags::mem_device);
        if (tid == 0) tg_old_energy = tg_energy_shared;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Scale direction (parallel)
        tg_reduce[tid] = 0.0f;
        for (int i = (int)tid; i < n_terms; i += (int)tg_size) tg_reduce[tid] += my_dir[i]*my_dir[i];
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint s = tg_size/2; s > 0; s >>= 1) { if (tid < s) tg_reduce[tid] += tg_reduce[tid+s]; threadgroup_barrier(mem_flags::mem_threadgroup); }
        float dir_norm = sqrt(tg_reduce[0]);
        if (dir_norm > max_step) {
        ...
        // Slope (parallel)
        tg_reduce[tid] = 0.0f;
        for (int i = (int)tid; i < n_terms; i += (int)tg_size) tg_reduce[tid] += my_dir[i]*my_grad[i];
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint s = tg_size/2; s > 0; s >>= 1) { if (tid < s) tg_reduce[tid] += tg_reduce[tid+s]; threadgroup_barrier(mem_flags::mem_threadgroup); }
        if (tid == 0) tg_slope = tg_reduce[0];
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Lambda min (parallel max)
        tg_reduce[tid] = 0.0f;
        for (int i = (int)tid; i < n_terms; i += (int)tg_size) { float tv = abs(my_dir[i])/max(abs(my_pos[i]),1.0f); tg_reduce[tid] = max(tg_reduce[tid],tv); }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        if (tid == 0) { tg_lambda_min = MOVETOL / max(tg_reduce[0], 1e-30f); tg_lam = 1.0f; tg_prev_lam = 1.0f; tg_prev_e = tg_old_energy; tg_ls_done = 0; }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Line search — thread 0 decides lambda, ALL threads update positions
        for (int ls = 0; ls < MAX_LS_ITERS && tg_ls_done == 0; ls++) {
            if (tg_lam < tg_lambda_min) {
                for (int i=(int)tid;i<n_terms;i+=(int)tg_size) my_pos[i]=my_old_pos[i];
                threadgroup_barrier(mem_flags::mem_device);
                if (tid == 0) tg_ls_done = 1;
                threadgroup_barrier(mem_flags::mem_threadgroup);
                break;
            }
            float cur_lam = tg_lam;
            for (int i=(int)tid;i<n_terms;i+=(int)tg_size) my_pos[i]=my_old_pos[i]+cur_lam*my_dir[i];
            threadgroup_barrier(mem_flags::mem_device);
            float trial_e = 0.0f;
            SEQ_COMPUTE_E(trial_e);
            // Thread 0 decides
            if (tid == 0) {
                float oe = tg_old_energy;
                float sl = tg_slope;
                if (trial_e - oe <= FUNCTOL*cur_lam*sl) {
                    tg_energy_shared = trial_e;
                    tg_ls_done = 1;
                } else {
                    float tmp;
                    float pl = tg_prev_lam, pe = tg_prev_e;
                    if (ls==0) { tmp=-sl/(2.0f*(trial_e-oe-sl)); }
                    else { float r1=trial_e-oe-cur_lam*sl, r2=pe-oe-pl*sl; float l2=cur_lam*cur_lam,pl2=pl*pl,d=cur_lam-pl;
                      if(abs(d)<1e-30f){tmp=0.5f*cur_lam;}else{float a=(r1/l2-r2/pl2)/d,b=(-pl*r1/l2+cur_lam*r2/pl2)/d;
                      if(abs(a)<1e-30f)tmp=(abs(b)>1e-30f)?-sl/(2.0f*b):0.5f*cur_lam;else{float disc=b*b-3.0f*a*sl;if(disc<0)tmp=0.5f*cur_lam;else if(b<=0)tmp=(-b+sqrt(disc))/(3.0f*a);else tmp=-sl/(b+sqrt(disc));}}}
                    tmp=min(tmp,0.5f*cur_lam); tmp=max(tmp,0.1f*cur_lam);
                    tg_prev_lam=cur_lam; tg_prev_e=trial_e; tg_lam=tmp;
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
        ...
        tg_reduce[tid]=0.0f;
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) { float tv=abs(my_old_pos[i])/max(abs(my_pos[i]),1.0f); tg_reduce[tid]=max(tg_reduce[tid],tv); }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint s=tg_size/2;s>0;s>>=1){if(tid<s)tg_reduce[tid]=max(tg_reduce[tid],tg_reduce[tid+s]);threadgroup_barrier(mem_flags::mem_threadgroup);}
        if (tg_reduce[0] < TOLX) { if(tid==0) tg_status_shared=0; threadgroup_barrier(mem_flags::mem_threadgroup); break; }

        // Save old grad, compute new (thread 0 does E+G)
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) my_dgrad[i]=my_grad[i];
        threadgroup_barrier(mem_flags::mem_device);
        float new_e = 0.0f;
        SEQ_COMPUTE_EG(new_e);
        if (tid == 0) tg_energy_shared = new_e;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) my_dgrad[i]=my_grad[i]-my_dgrad[i];
        threadgroup_barrier(mem_flags::mem_device);

        // Grad convergence (parallel max)
        tg_reduce[tid]=0.0f;
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) { float tv=abs(my_grad[i])*max(abs(my_pos[i]),1.0f); tg_reduce[tid]=max(tg_reduce[tid],tv); }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint s=tg_size/2;s>0;s>>=1){if(tid<s)tg_reduce[tid]=max(tg_reduce[tid],tg_reduce[tid+s]);threadgroup_barrier(mem_flags::mem_threadgroup);}
        if (tg_reduce[0]/max(tg_energy_shared,1.0f) < grad_tol) { if(tid==0) tg_status_shared=0; threadgroup_barrier(mem_flags::mem_threadgroup); break; }

        // BFGS: hessDGrad = H @ dGrad (ALL threads, parallel rows)
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) { float s=0; for(int j=0;j<n_terms;j++) s+=my_H[i*n_terms+j]*my_dgrad[j]; my_hess_dg[i]=s; }
        threadgroup_barrier(mem_flags::mem_device);
        ...
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) { tg_fac[tid]+=my_dgrad[i]*my_old_pos[i]; tg_fae[tid]+=my_dgrad[i]*my_hess_dg[i]; tg_sdg[tid]+=my_dgrad[i]*my_dgrad[i]; tg_sxi[tid]+=my_old_pos[i]*my_old_pos[i]; }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint s=tg_size/2;s>0;s>>=1){if(tid<s){tg_fac[tid]+=tg_fac[tid+s];tg_fae[tid]+=tg_fae[tid+s];tg_sdg[tid]+=tg_sdg[tid+s];tg_sxi[tid]+=tg_sxi[tid+s];}threadgroup_barrier(mem_flags::mem_threadgroup);}
        ...
        if (fac*fac>EPS_GUARD*sum_dg*sum_xi && fac>0) {
            ...
            for (int k=(int)tid;k<hess_total;k+=(int)tg_size) {
                int i=k/n_terms, j=k%n_terms;
                float xi_i=my_old_pos[i],xi_j=my_old_pos[j],hd_i=my_hess_dg[i],hd_j=my_hess_dg[j];
        ...
        for (int i=(int)tid;i<n_terms;i+=(int)tg_size) { float s=0; for(int j=0;j<n_terms;j++) s+=my_H[i*n_terms+j]*my_grad[j]; my_dir[i]=-s; }
        threadgroup_barrier(mem_flags::mem_device);
    }

    if (tid == 0) { out_energies[mol_idx]=tg_energy_shared; out_statuses[mol_idx]=tg_status_shared; }
"""
